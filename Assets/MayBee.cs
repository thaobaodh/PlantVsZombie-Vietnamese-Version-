...
public class MayBee : MonoBehaviour
{
    ...
    void Shoot()
    {
        ...
        canShoot = false;
        for(int i = 0; i < shootOrigin.Length; i++)
        {
            GameObject myBullet = Instantiate(bullet, shootOrigin[i].position, Quaternion.identity);
            myBullet.GetComponent<Bee>().HomePosition = this.transform;
        }
    }
}
